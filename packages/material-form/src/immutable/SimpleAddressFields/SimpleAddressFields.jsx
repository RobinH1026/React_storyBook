...
const SimpleAddressFields = (props) => {
  ...
  const { onChange: cityInputOnChange, ...otherCityInput } =
    cityFormProps.input;
  const { onChange: areaInputOnChange, ...otherAreaInput } =
    areaFormProps.input;
  ...
  const cities = useMemo(() => data.map((el) => el.get("city")), [data]);
  const [dists, setDists] = useState(List());
