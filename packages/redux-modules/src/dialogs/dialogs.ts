...
export const dialogs = createReducer(initialState, {
  [INITIALIZE_DIALOG]: produce((draft, action) => {
    ...
    if (!isSupported) {
      warning(
        false,
        `[@eGroupAI/redux-modules] ERROR: Action "initializeDialog" is not supported "${type}" payload.`
      );
      return;
    }
  ...
  [OPEN_DIALOG]: produce((draft, action) => {
    ...
    if (!isSupported) {
      warning(
        false,
        `[@eGroupAI/redux-modules] ERROR: Action "openDialog" is not supported "${type}" payload.`
      );
      return;
    }
  ...
  [CLOSE_DIALOG]: produce((draft, action) => {
    ...
    if (!isSupported) {
      warning(
        false,
        `[@eGroupAI/redux-modules] ERROR: Action "closeDialog" is not supported "${type}" payload.`
      );
      return;
    }
    const name = action.payload;
  ...
  [SET_DIALOG_DATA]: produce((draft, action) => {
    ...
    if (!isSupported) {
      warning(
        false,
        `[@eGroupAI/redux-modules] ERROR: Action "setDialogData" is not supported "${type}" payload.`
      );
      return;
    }
    ...
    if (hasIn(draft, [name])) {
      ...
      warning(
        false,
        `[@eGroupAI/redux-modules] ERROR: Dialog "${name}" is not exist.`
      );
    }
  }),
});
