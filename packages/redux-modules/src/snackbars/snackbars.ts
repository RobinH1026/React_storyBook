...
import hasIn from "@eGroupAI/utils/hasIn";
...
export const snackbars = createReducer(initialState, {
  [INITIALIZE_SNACKBAR]: produce((draft, action) => {
    ...
    if (!isSupported) {
      warning(
        false,
        `[@eGroupAI/redux-modules] ERROR: Action "initializeSnackbar" is not supported "${type}" payload.`
      );
      return;
    }
  ...
  [OPEN_SNACKBAR]: produce((draft, action) => {
    ...
    if (!isSupported) {
      warning(
        false,
        `[@eGroupAI/redux-modules] ERROR: Action "openSnackbar" is not supported "${type}" payload.`
      );
      return;
    }
  ...
  [CLOSE_SNACKBAR]: produce((draft, action) => {
    ...
    if (!isSupported) {
      warning(
        false,
        `[@eGroupAI/redux-modules] ERROR: Action "closeSnackbar" is not supported "${type}" payload.`
      );
      return;
    }
    const name = action.payload;
  ...
  [SET_SNACKBAR_DATA]: produce((draft, action) => {
    ...
    if (!isSupported) {
      warning(
        false,
        `[@eGroupAI/redux-modules] ERROR: Action "setSnackbarData" is not supported "${type}" payload.`
      );
      return;
    }
    ...
    if (hasIn(draft, [name])) {
      ...
      warning(
        false,
        `[@eGroupAI/redux-modules] ERROR: Snackbar "${name}" is not exist.`
      );
    }
  }),
});
