...
import deleteIn from "@eGroupAI/utils/deleteIn";
...
export const apis = createReducer(initialState, {
  ...
  [EG_CLEAR_API_RESPONSE]: produce((draft, action) => {
    ...
    if (!isSupported) {
      warning(
        false,
        `[@eGroupAI/redux-modules] ERROR: Action "clearApiResponse" is not supported "${type}" payload.`
      );
      return;
    }
    ...
    if (typeof actionType !== "string") {
      warning(
        false,
        `[@eGroupAI/redux-modules] ERROR: Redux action type need to be "string" not "${typeof actionType}"`
      );
      return;
    }
  ...
  [EG_CLEAR_APIS_RESPONSE]: produce((draft, action) => {
    ...
    if (!isSupported) {
      warning(
        false,
        `[@eGroupAI/redux-modules] ERROR: Action "clearApisResponse" is not supported ${type} payload.`
      );
      return;
    }
  ...
  [EG_DESTROY_API]: produce((draft, action) => {
    ...
    if (!isSupported) {
      warning(
        false,
        `[@eGroupAI/redux-modules] ERROR: Action "destroyApi" is not supported ${type} payload.`
      );
      return;
    }
