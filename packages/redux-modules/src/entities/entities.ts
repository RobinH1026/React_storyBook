  ...
  [SET_ENTITIES]: produce((draft, action) => {
    ...
    if (action.meta) {
      ...
      if (!isSupported) {
        warning(
          false,
          `[@eGroupAI/redux-modules] ERROR: Action "setEntities" is not supported "${type}" payload.`
        );
        return;
      }
      setIn(
  ...
  [SET_ENTITIES_SHALLOW]: produce((draft, action) => {
    ...
    if (action.meta) {
      ...
      if (!isSupported) {
        warning(
          false,
          `[@eGroupAI/redux-modules] ERROR: Action "setEntitiesShallow" is not supported "${type}" payload.`
        );
        return;
      }
      setIn(
